...
export default function ChooseNetworkFee({
  ...
  return (
    <>
      <div className={cn('mx-auto flex w-full gap-2 max-sm:flex-col text-black1')}>
        {(['normal', 'high', 'custom'] as NetworkFeeType[]).map((network) => (
          ...
          >
            <div className="mb-1 text-sm font-medium capitalize">{network}</div>
            <div>
              {loading ? (
                'loading'
              ) : networkFee ? (
                <p className="flex font-ProtoMono  gap-1">
                  <span className="font-bold text-red-light text-base leading-3/2">
                    {network !== 'custom' ? networkFee[network] : customNetworkFee}
                  </span>
                  <span className="text-black1 font-light text-base leading-3/2">sats/vB</span>
                </p>
              ) : (
                'empty'
              )}
