...
const columns: Array<ColumnProps<Owner>> = [
  {
    ...
    title: '#',
    render: (_, record) => {
      return <p className="relative flex w-[60px] items-center justify-center text-left">{record.id}</p>
    },
    renderColumn: (_) => {
      return <p className="relative flex w-[60px] items-center justify-center text-left">{_.title}</p>
    },
  },
  {
    key: 'name',
      ...
      return (
        <div className="relative flex w-[117px] items-center space-x-[10px] px-4 py-2 text-left text-[#4E473F] md:w-[300px]">
          {record.avatar ? (
            ...
            <>
              <Image src={avt} alt="avatar" width={40} height={40} className="min-size-10 max-size-10 rounded" />
            </>
          )}
  ...
  },
  {
    ...
    title: 'Owned',
    render: (_, record) => {
      return <p className="relative grow text-end font-light text-[#4E473F]">{record.Count.toLocaleString()}</p>
    },
    renderColumn: (_) => {
      return (
        <p className="relative grow text-end font-medium text-[#4E473F]">
          <Trans>{_.title}</Trans>
        </p>
      )
    },
  ...
  return (
    <div className="relative rounded-lg bg-[#fff] py-4 lg:h-[1200px] lg:py-12 shadow-[0px_4px_15px_0px_rgba(0,0,0,0.06)]">
      ...
      <div className="relative mx-auto w-full space-y-8 lg:w-[920px]">
        ...
        {Number(owners?.totalPages) > 0 && <Pagination pageCount={page} setPage={setPage} itemsPerPage={PAGE_SIZE} />}
      </div>
    </div>
  )
}
